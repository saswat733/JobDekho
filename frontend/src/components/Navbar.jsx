...
const Navbar = () => {
  ...
  return (
    ...
    <div className="w-full fixed z-10">
      ...
      <div
        className={`text-black flex flex-col text-left bg-white transition-all w-full pt-4 absolute duration-1000 ${
      ...
      >
        {navLinks.map((ele, key) => (
          <div key={key} className="w-fit text-center  p-4">
            <NavLink
              className={({isActive})=>isActive?activeStyle:"text-gray-500 hover:text-green-700"}
              to={ele.path} 
            >
              {ele.title}
            </NavLink>
